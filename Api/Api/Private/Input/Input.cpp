...
void Input::Tick(CDIManipulateur& cdiManipulateur)
{
    ...
    for (const KeyCode& keyCode : KeyCodeValues)
    {
        ...
        {
            if (currentlyHeldKeys.find(keyCode) != currentlyHeldKeys.end())
            {
                continue;
            }

            if (currentlyPressedKeys.find(keyCode) != currentlyPressedKeys.end())
            {
        ...
        {
            if (currentlyHeldKeys.find(keyCode) != currentlyHeldKeys.end() || currentlyPressedKeys.find(keyCode) != currentlyPressedKeys.end())
            {
                currentlyPressedKeys.erase(keyCode);
...
{
    return currentlyReleasedKeys.find(keyCode) != currentlyReleasedKeys.end();
}

bool Input::isKeyHeld(const KeyCode keyCode) const
{
    return currentlyHeldKeys.find(keyCode) != currentlyHeldKeys.end();
}
...
{
    keyPressedListeners.erase(
    std::remove_if(keyPressedListeners.begin(), keyPressedListeners.end(),
        [&](const std::function<void(KeyCode&)>& f) {
            return f.target<std::function<void(KeyCode&)>>() == listener.target<std::function<void(KeyCode&)>>();
        }),
    keyPressedListeners.end());
}

void Input::removeKeyReleasedListener(const std::function<void(KeyCode&)>& listener)
{
    keyReleasedListeners.erase(
    std::remove_if(keyReleasedListeners.begin(), keyReleasedListeners.end(),
        [&](const std::function<void(KeyCode&)>& f) {
            return f.target<std::function<void(KeyCode&)>>() == listener.target<std::function<void(KeyCode&)>>();
        }),
    keyReleasedListeners.end());
}
