...
namespace PM3D_API
{
    ...
    void UIObject::SetCentralRotation(const float newRotation)
    {
        ...
        center.x = basePositionComputed.x + (baseScaleComputed.x / 2.0f);
        center.y = basePositionComputed.y + (baseScaleComputed.y / 2.0f);
        ...
        float translatedX = basePositionComputed.x - center.x;
        float translatedY = basePositionComputed.y - center.y;
